...
--- Add a new bone to a bone layer's skeleton.
-- @param params table  Must contain layerId. Optional: name, parentBoneId, posX, posY, angle, length, scale.
function bone.addBone(moho, params)
    ...
    if not castOk or not boneLyr then
        return nil, "Failed to cast layer to bone layer"
    end

    local skelOk, skel = pcall(function() return boneLyr:Skeleton() end)
    if not skelOk or not skel then
        return nil, "Failed to get skeleton"
    end
    ...
    local newIndex = skel:CountBones() - 1
    ...
    if params.posX ~= nil or params.posY ~= nil then
        pcall(function()
            local vec = LM.Vector2:new_local()
            vec.x = params.posX or 0
            vec.y = params.posY or 0
            newBone.fPos = vec
        end)
    end
    if params.angle ~= nil then
        pcall(function() newBone.fAngle = params.angle end)
    end
    if params.length ~= nil then
        pcall(function() newBone.fLength = params.length end)
    end
    if params.scale ~= nil then
        pcall(function() newBone.fScale = params.scale end)
    end
    if params.parentBoneId ~= nil and type(params.parentBoneId) == "number" then
        pcall(function() newBone.fParent = params.parentBoneId end)
    end
...
--- Set the rest-pose properties of a bone (position, angle, length, scale, parent, name).
-- Unlike bone.setTransform which animates the bone at a frame, this changes the
-- bind/rest pose directly. Useful right after creating a bone.
function bone.setRestPose(moho, params)
    if not params or params.layerId == nil or params.boneId == nil then
        return nil, "Missing required parameter: layerId and boneId"
    end
    ...
    if params.name ~= nil and type(params.name) == "string" then
        pcall(function() b:SetName(params.name) end)
        changed.name = params.name
    end
    if params.posX ~= nil or params.posY ~= nil then
        local ok = pcall(function()
            local vec = LM.Vector2:new_local()
            vec.x = params.posX or (b.fPos and b.fPos.x) or 0
            vec.y = params.posY or (b.fPos and b.fPos.y) or 0
            b.fPos = vec
        end)
        if ok then
            changed.posX = params.posX
    ...
    if params.angle ~= nil then
        pcall(function() b.fAngle = params.angle end)
        changed.angle = params.angle
    end
    if params.length ~= nil then
        pcall(function() b.fLength = params.length end)
        changed.length = params.length
    end
    if params.scale ~= nil then
        pcall(function() b.fScale = params.scale end)
        changed.scale = params.scale
    end
    if params.parentBoneId ~= nil then
        pcall(function() b.fParent = params.parentBoneId end)
        changed.parentBoneId = params.parentBoneId
    end
