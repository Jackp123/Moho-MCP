...
-- Per the ScriptInterface docs: moho:FileSave() and moho:FileSaveAs(path).
...
function document.save(moho, params)
    ...
    local targetPath = params.filePath
    if targetPath ~= nil and type(targetPath) ~= "string" then
    ...
    local actualPath = ""
    pcall(function() actualPath = doc:Path() end)
